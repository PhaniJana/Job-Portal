...
router.post("/upload", async (req, res) => {
    try {
        ...
        return res.json({url:cloud.url,public_id:cloud.public_id});
    } catch (error : any) {
...
router.post('/resume-analyzer',async(req,res)=>{
    try {
        const { pdfBase64, pdxfBase64 } = req.body;
        const encodedPdf = pdfBase64 ?? pdxfBase64;
        if(!encodedPdf){
            return res.status(400).json({message:"PDF base64 string is required"});
        }
        const prompt = `
        ...
        const response = await ai.models.generateContent({
                ...
                },
            {
                inlineData:{
                    ...
                    data: encodedPdf.replace(/^data:application\/pdf;base64,/, '')
                }
            }
        ]
            }],
