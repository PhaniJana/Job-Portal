        ...
        <CardHeader className='space-y-4 pb-4'>
            <div className="flex items-start justify-between gap-3">
                ...
                <Link href={`/company/${job.company_id}`} className="shrink-0">
                    <div className="w-14 h-14 rounded-xl border-2 overflow-hidden hover:scale-105 transition-transform bg:background">
                        <Image width={56} height={56} src={job.company_logo} alt={job.company_name} />
                    </div>
                </Link>
            </div>
            <div className="space-y-2">
                ...
                <div className="flex items-center gap-2 text-base font-semibold">
                    ...
                    <span className='font-bold'>${job.salary?.toLocaleString()} P.A</span>
                </div>
            </div>
        </CardHeader>
        <CardContent className='flex flex-col gap-3 pt-4 border-t'>
            <div className="flex w-full gap-2">
                ...
                </Link>
                {
                    ...
                    <>
                    {
                        applied ? <div className="flex-1 flex items-center justify-center gap-2 bg-green-100 text-green-600 rounded px-3 py-1.5">
                            <CheckCircle size={18} />Applied
                        </div>:<>
                        {job.is_active !== false &&
                            <Button disabled={btnLoading} className='flex-1 gap-2' onClick={()=>handleApply(job.job_id)}>
            ...
            {
                job.is_active === false && <div className="text-sm text-red-600 text-center font-medium">
